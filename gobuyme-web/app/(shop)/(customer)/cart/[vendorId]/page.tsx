...
export default function VendorCartPage() {
  ...
  const { getVendorCart, updateQty, removeItem } = useCart();
  ...
  if (items.length === 0) return (
    <div className="page-body"><div className="inner">
      <div className="empty" style={{ padding: '80px 24px' }}>
        ...
        <p>Add items from this vendor to get started.</p>
        <Link href="/cart" className="btn btn-primary" style={{ marginTop: 24 }}>Back to Carts</Link>
      </div>
    </div></div>
  );
      ...
      <div className="inner">
        ...
        <div className="layout-grid">
              ...
              return (
                <div key={key} style={{ display: 'flex', gap: 14, padding: '16px 20px', borderBottom: '1px solid var(--line)', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <button onClick={() => updateQty(vendorId, key, item.qty - 1)} style={{ width: 32, height: 32, borderRadius: '50%', border: '1.5px solid var(--line)', background: 'var(--surface)', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontWeight: 700 }}>−</button>
                    <span style={{ fontWeight: 700, minWidth: 20, textAlign: 'center' }}>{item.qty}</span>
                    <button onClick={() => updateQty(vendorId, key, item.qty + 1)} style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--brand)', color: '#fff', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontWeight: 700, border: 'none' }}>+</button>
                  </div>
                  <div style={{ fontWeight: 800, minWidth: 80, textAlign: 'right' }}>₦{(item.price * item.qty).toLocaleString()}</div>
                  <button onClick={() => removeItem(vendorId, key)} style={{ color: 'var(--error)', marginLeft: 8, fontSize: 18, background: 'none', border: 'none', cursor: 'pointer' }}>✕</button>
                </div>
              );
            })}
          </div>
          ...
          <div className="card card-pad layout-sticky">
            ...
            <button className="btn btn-primary btn-block btn-lg" onClick={() => router.push(`/checkout?vendorId=${vendorId}`)}>
              Proceed to Checkout →
            </button>
