              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 8 }}>
                  {item.drinkOptions.filter(d => d.isAvailable).map(d => (
                    <div key={d.id} style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '10px 12px', borderRadius: 'var(--r)', border: '1px solid var(--line)', background: 'var(--surface)' }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, lineHeight: 1.3 }}>{d.name}</div>
                        <div style={{ fontSize: 11, color: 'var(--brand)', fontWeight: 600, marginTop: 2 }}>+₦{d.price.toLocaleString()}</div>
                      </div>
              ...
              <div key={g.id} style={{ marginBottom: 18 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 8 }}>
                  {g.items.filter(i2 => i2.isAvailable).map(i2 => (
                    <div key={i2.id} style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '10px 12px', borderRadius: 'var(--r)', border: '1px solid var(--line)', background: 'var(--surface)' }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, lineHeight: 1.3 }}>{i2.name}</div>
                        {i2.extraPrice > 0 && <div style={{ fontSize: 11, color: 'var(--brand)', fontWeight: 600, marginTop: 2 }}>+₦{i2.extraPrice.toLocaleString()}</div>}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
