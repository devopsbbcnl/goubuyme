        ...
        {(['TIER_1', 'TIER_2'] as const).map(t => (
          <div key={t} style={{
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--brand)', marginBottom: 6 }}>
              {rates[t].platformPercent}% <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--muted)' }}>commission</span>
            </div>
      ...
      {confirm && (
        <div style={{ padding: '14px 16px', background: 'var(--surface2)', borderRadius: 4, border: '1px solid var(--line)' }}>
          <p style={{ fontSize: 13, marginBottom: 12 }}>
            {confirm === 'TIER_2'
              ? `Upgrading to Tier 2 changes your commission to ${rates.TIER_2.platformPercent}% and unlocks promotional cards. A 14-day cooldown applies after switching.`
              : `Downgrading to Tier 1 changes your commission to ${rates.TIER_1.platformPercent}% but deactivates all your promotional cards. A 14-day cooldown applies after switching.`}
          </p>
          <div style={{ display: 'flex', gap: 8 }}>
